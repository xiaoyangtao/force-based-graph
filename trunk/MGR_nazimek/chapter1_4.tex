...
Komunikacja odbywa siê asynchronicznie, po umieszczeniu komunikatu w kanale,
...
bezprzewodowej utrata po³¹czenia jest czymœ czego mo¿emy siê
spodziewaæ.\\
\indent
...
pod uwagê. G³ówn¹ przeszkod¹ jest brak uniwersalnego sposobu pod³¹czenia do bazy
...
Okazuje siê, ¿e dwoma godnymi rozwa¿enia metodami s¹ wiadomoœci oraz Web
serwisy, bêd¹ce jednym ze sposobów zdalnego wywo³ywania procedur.
...
le¿y w zastosowaniu kilku powszechny technologii internetowych, takich jak
protokó³ HTTP czy jêzyk XML. To gwarantuje, ¿e nie ma znaczenia w jakiej
technologii i w jakim systemie pracuje dana aplikacja komunikuj¹ca siê za
pomoc¹ Web Services. Trzy fundamentalne elementy wchodz¹ce w sk³ad tej
techlogii mo¿na zobaczyæ na poni¿szym rysunku.
...
elektronicznej koperty, która zawiera podstawowe informacje o rz¹daniu oraz o
odpowiedzi. Jest zbudowana w oparciu o jêzyk XML. Drug¹ kluczow¹ technologi¹
jest jêzyk opisu us³ugi - Web Service Description Language (WSDL). Za jego
...
wiadomoœci przesy³ana jest w zoptymalizownej postaci kodu binarnego i nie
...
s¹ w kopercie zbudowanej na bazie XML, która posiada du¿o nadmiernych
informacji. Powoduje to, ¿e przy przesy³aniu wiadomœci ³¹cze dodatkowo zostaje
...
jakie on generuje dla ca³ej organizacji.
\newline
Z myœl¹ o bezpiecznej komunikacji przy u¿yciu technologii Web Service
...
które dzia³aj¹ praktycznie na ka¿dej platformie wspieraj¹cej wirtualn¹ maszyne
Java. Pozosta³e platformy taki jak Blackberry czy WMD (Windows Mobile Device)
równie¿ posiadaj¹ stosowne biblioteki, które pozwalaj¹ w bardzo prosty sposób
pod³¹czyæ siê do istniej¹cej infrastuktury. Znane s¹ komercyjne rozwi¹zania
...
³¹cza czy te¿ gwarancja dostaczenia, to jest to technologia, której u¿ycie jest
...
problemów,
które musi samodzielnie rozwi¹zaæ. Jest to przede wszystkim brak gwarancji dostarczenia komunikatu. Dodatkowo
jednak samodzielnie nale¿y obs³u¿yæ b³êdy transmisji, mo¿liwe przerwy 
i koniecznoœæ ponownego nawi¹zania po³¹czenia. Z drugiej jednak
strony nie ma potrzeby uruchamiania brokera wiadomoœci, który jest dodatkowym elementem
zwiêkszaj¹cym skomplikowanie systemu. Jednak w sytuacji, gdy w przedsiêbiorstwie
dzia³a ju¿ broker obs³uguj¹cy kilka aplikacji, do³¹czenie komponentu mobilnego
mo¿e byæ dobrym rozwi¹zaniem. Zastosowanie wiadomoœci do integracji urz¹dzeñ
mobilnych niesie ze sob¹ wszelkie konsekwencje tej metody integracji znane z
integracji klasycznych systemów informatycznych:
...
\begin{itemize}
  ...
  \item Gwarancja dostarczenia. Umieszczenie wiadomoœci w kanale komunikacyjnym
  gwarantuje, ¿e odbiorca odbierze j¹ wtedy gdy bêdzie pobiera³ przeznaczone
  ...
  przetwarzanie, konwertowanie, zmienianie kolejnoœci i inne modyfikacje przesy³anych wiadomoœci. Pod³¹czenie do kana³u publish-subscribe
  pozwala na ³atwe wysy³anie wiadomoœci do wielu odbiorców.
\end{itemize}
Nie jest to, oczywiœcie, technologia pozbawiona wad. Tym co musimy wzi¹æ pod
uwagê jest istnienie wspomnianego ju¿ brokera wiadomoœci. Dodatkowo, wszystkie
wspomniane wczeœniej mo¿liwoœci maj¹ swój koszt, jakim jest wiêkszy rozmiar
aplikacji mobilnej wynikaj¹cy z koniecznoœci skorzystania z dodatkowych
komponentów. Nie jest to du¿y narzut w przypadku klasycznych aplikacji, jednak
zwiêksza rozmiary aplikacji mobilnej w stopniu, który mo¿e nie pozwoliæ na jej
uruchomienie na prostszych urz¹dzeniach.\\
\indent
...
Windows Mobile maj¹ swoje w³asne rozwi¹zania. Tym na co warto by zwróciæ uwagê,
jest fakt, ¿e stosowanie wiadomoœci umo¿liwia zmianê najni¿szej warstwy
...
zarówno w ten sposób jak i za pomoc¹ SMS-ów. Pozwala to na funkcjonowanie
aplikacji bez utrzymywania sta³ego po³¹czenia z internetem, co w przypadku
ograniczonej ³¹cznoœci jest mocno utrudnione. 


% Istnieje kilku dostawców
% rozwi¹zañ opartych o wiadomoœci z których najpopularniejsze to IBM MQ Everyplace
% oraz iBus//Mobile.
...
jednoznacznej odpowiedzi. Zale¿y to od oczekiwañ u¿ytkownika od integrowanego
systemu, jego natury oraz aktualnie istniej¹cych komponentów. Wydaje
...
\begin{itemize}
  ...
  \item Czy bierzemy pod uwagê znaczne wydatki, które wynikaj¹ z wykorzystania
  kosztownych, komercyjnych brokerów wiadomoœci? Wykorzystanie Web serwisów
  pozwala na znaczne obni¿enie kosztów, wynikaj¹ce z obecnoœci wielu
...
\end{itemize}
 ...
 Po przeanalizowaniu obu
metod komunikacji,w czêœci implementacyjnej naszej pracy zdecydowaliœmy siê na wykorzystanie Web serwisów z kilku powodów:
\begin{itemize}
  \item Wykorzystanie komercyjnego oprogramowania nie by³o mo¿liwe,a dodatkowo
  wymaga³oby uruchomienia skomplikowanej czêœci po stronie serwerowej. Naszym celem nie by³o konfigurowanie jednej skomplikowanej
platformy, ale raczej poznanie problemów, z którymi nale¿y siê liczyæ
integruj¹c aplikacjê mobiln¹.  
  \item Wykorzystanie powszechnie dostêpnego pakietu kJORAM (bezp³atnej
  implementacji JMS dostêpnej tak¿e dla urz¹dzeñ mobilnych) oznacza³o tak naprawdê
  wykorzystanie Web serwisów, które zosta³y odpowiednio obudowane przez twórców szablonu.
